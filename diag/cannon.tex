...
\begin{tikzpicture}
    ...
    \begin{scope}[decoration={markings, mark=at position 0.5 with {\arrow{>}}}]     % All lines in this scope have an arrow in the middle
        ...
        \draw[postaction={decorate}] (S) -- (SC) .. controls ($(SC) + (2,0)$) and ($(EC) + (0,3)$) ..  node[pos=0.35, anchor=north, yshift=-2pt] {$(C)$} (EC);
        \draw[postaction={decorate}] (SC) -- (SD) .. controls ($(SD) + (1.5,0)$) and ($(ED) + (0,1.5)$) ..  node[pos=0.55, anchor=north, yshift=-2pt] {$(D)$} (ED) -- (EC);
        \draw[postaction={decorate}] (SD) -- (SE) .. controls ($(SE) + (1,0)$) and ($(EE) + (0,1)$) ..  node[pos=0.95, anchor=west, xshift=-2pt] {$(E)$} (EE) -- (ED);
    \end{scope}
\end{tikzpicture}
